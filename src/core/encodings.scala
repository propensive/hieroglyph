...
object Encoding:
  given (Textualizer { type Self = Encoding }) = _.name
  given (Communicable { type Self = Encoding }) = encoding => Message(encoding.name)
...
object Hieroglyph:
  ...
  object CharRange:
    ...
    given show: (Textualizer { type Self = CharRange }) = range => "${range.from}..${range.to}".tt
